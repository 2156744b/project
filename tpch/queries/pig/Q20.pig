...
part2 = filter part1 by SUBSTRING(p_name, 0, 4)=='drab';
part3 = foreach part2 generate p_partkey;
part4 = distinct part3;

lineitem = LOAD '/apps/hive/warehouse/lineitem' USING OrcStorage('|') as (l_orderkey, l_partkey:long, l_suppkey:long, l_linenumber, l_quantity:double, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate:chararray, l_commitdate, l_receiptdate,l_shippingstruct, l_shipmode, l_comment);
lineitem1 = foreach lineitem generate l_partkey, l_suppkey, l_shipdate, l_quantity;
lineitem2 = filter lineitem1 by l_shipdate >= '1996-01-01' and l_shipdate < '1997-01-01';
...
partsupp = LOAD '/apps/hive/warehouse/partsupp' USING OrcStorage('|') as (ps_partkey:long, ps_suppkey:long, ps_availqty:long, ps_supplycost, ps_comment);
...
nation1 = filter nation by n_name == 'INDIA';

supplier = LOAD '/apps/hive/warehouse/supplier' USING OrcStorage('|') as (s_suppkey:long, s_name:chararray, s_address:chararray, s_nationkey:int, s_phone, s_acctbal, s_comment);
...
store res into '/apps/hive/warehouse/q20' USING OrcStorage('|');
