...
fregion = filter region by r_name == 'AFRICA';
forders = filter orders by o_orderdate <= '1996-12-31' and o_orderdate >= '1995-01-01';
fpart = filter part by p_type == 'STANDARD POLISHED COPPER';
...
sels1 = foreach l1 generate SUBSTRING(o_orderdate,0,4) as o_year, volume, (n_name == 'ETHIOPIA'? volume : 0) as case_volume;
...
store sortResult into '/apps/hive/warehouse/q8' USING OrcStorage('|');
